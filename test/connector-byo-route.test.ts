...
test("admin stores validated Slack tokens without ever returning them", async () => {
  ...
  try {
    ...
    assert.equal(JSON.parse(createUrl.searchParams.get("manifest_json")!).display_information.name, BRAND.slackAppName);
...
test("the catalog endpoint exposes per-provider setup guidance (no secrets)", async () => {
  ...
  try {
    ...
    assert.deepEqual(names, ["dropbox", "github", "google", "linear", "microsoft", "notion", "slack", "x"]);
