...
import { BRAND_LOGO_PATH } from "../../chassis/src/brand.ts";
...
export function renderSignInEmail(args: {
  ...
  const logo = logoUrl(link);
  const logoImg = logo
    ? `<img src="${escapeHtml(logo)}" width="40" height="40" alt="${escapeHtml(brand)}" style="display:block;margin:0 0 16px;border:0">\n`
    : "";
  const text = [
    `Sign in to ${brand}`,
    "",
...
<tr><td>
${logoImg}<h1 style="margin:0 0 10px;font-size:20px;font-weight:600">Sign in to ${escapeHtml(brand)}</h1>
